...
for i in $SAMPLES; do awk -F'\t' '{print $1}' coordinates/minus/XM_036492600.1_coord.txt | samtools faidx refgenome/GCF_014633375.1_OchPri4.0_genomic.fna -r - | bcftools consensus -s "$i" 41_norm_dp4_renamed_ig5.vcf.gz -I -M "N" > nuc_fasta/minus/XM_036492600.1/"$i"_XM_036492600.1.fa
done
#Then concatenate the fasta records for each exon within each fasta file
#for infile in coordinates/minus/*.txt; do f=$(basename ${infile} _coord.txt); for infile in nuc_fasta/minus/"$f"/*.fa; do j=$(basename ${infile} .fa); sed -s '1h;/>/d;H;$!d;x;s/\n//2g' nuc_fasta/minus/"$f"/"$j".fa > nuc_fasta/minus/"$f"/"$j"_cat.fa
#done; done
#Then did this to get the sample names into the fasta header
#for infile in coordinates/minus/*.txt; do f=$(basename ${infile} _coord.txt); for infile in nuc_fasta/minus/"$f"/*_cat.fa; do j=$(basename ${infile} _cat.fa); sed -i "s/^>/>${f%_*}_/" nuc_fasta/minus/"$f"/"$j"_cat.fa
#done; done
#Then combined each sample's fasta into multisample gene alignments. 
#for infile in coordinates/minus/*.txt; do f=$(basename ${infile} _coord.txt); cat nuc_fasta/minus/"$f"/*_cat.fa > nuc_fasta/cat_minus/"$f".fa
#done
#Then rename the sample headers in the fasta file so the genes all match and with more descriptive names
#for infile in coordinates/minus/*.txt; do f=$(basename ${infile} _coord.txt); awk 'NR%2==0' nuc_fasta/cat_minus/"$f".fa | paste -d'\n' nuc_fasta_rename.txt - > nuc_fasta/cat_minus/"$f"_renamed.fa; done
